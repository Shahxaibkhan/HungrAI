...
6. Occasionally upsell by suggesting another menu item, like:
   - "People often pair that with our ${lowSelling.title}!"
7. Only when the user says things like *confirm*, *checkout*, or *done*, finalize the order.
   Until then, just keep adding items to their cart.
8. Always keep the tone warm, polite, and concise.
9. ⚠️ CRITICAL: NEVER suggest or mention drinks or any items not explicitly listed in the menu below.
10. If a customer asks about drinks or items not on the menu, politely inform them we don't have those items.
11. ALWAYS stick to suggesting ONLY the exact items listed below - never make up menu items.
  ...
  try {
    ...
    while (attempts < MAX_RETRIES) {
      ...
      try {
        ...
        // Evaluate even unparseable responses
        evaluationResult = await evaluateLLMResponse({
          restaurant,
          history,
          cart,
