  ...
  try {
    ...
    if (/\b(confirm|checkout|place|done|finish)\b/.test(lowerMsg)) {
      ...
      if (checkoutSession.cart.length === 0) {
        ...
        const previousAssistantMessages = historyMessages.filter(msg => msg.role === 'assistant');
        
        // Look for messages suggesting items were added
    ...
    if (/\b(yes|sure|ok|okay|yup|yeah|add)\b/.test(lowerMsg) &&
      ...
      if (lastAssistantMsg && (
        ...
        let quantitySourceMsg = lowerMsg; // default to current
        ...
        const quantityPatterns = [
          /(\d+)\s*(fires?|fries?|burgers?|chickens?)/gi,
          /(one|two|three|four|five)\s*(fires?|fries?|burgers?|chickens?)/gi
        ];
        ...
        if (itemsToAdd.length === 0) {
          ...
          // If we still don't have any items and there are suggestions, use all of them
          if (itemsToAdd.length === 0 && session.lastSuggested.length > 0) {
    ...
    if (replyText.includes('added') || 
        replyText.includes('your cart') || 
        replyText.includes('your order')) {
        
      console.log('Natural language response mentions cart/order - attempting to parse items');
    ...
    if (!itemsUpdated && lowerMsg.match(/(burger|fries|fires|chicken)/i)) {
