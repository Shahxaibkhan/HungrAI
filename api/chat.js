...
const { buildPromptAndCallLLM } = require('./promptOrchestrator');
...
// Use /tmp in serverless environment (Netlify)
// ===============================
const SESSION_DIR = path.join('/tmp', 'sessions');
const MAX_CONTEXT = 8;
const SESSION_TIMEOUT_MS = 10 * 60 * 1000;

// Ensure the sessions directory exists (will be created on first use in /tmp)
// Note: /tmp is the only writable directory in Netlify serverless functions
...
function getSession(key) {
  ...
  try {
    // Ensure sessions directory exists before accessing it
...
async function chatHandler(req, res) {
  ...
  try {
    ...
    if (!slug) {
      console.error('[CHAT] Missing slug in query params');
      return res.status(400).json({ error: 'Restaurant slug is required', receivedQuery: req.query });
    }
    ...
    // BUT NOT if user is just clarifying what they already ordered
    // =========================
    const isJustClarifying = /\b(i ordered|i wanted|i got|i have|already ordered|what.*in.*cart|how many)\b/i.test(lowerMsg);
    
    if (!isJustClarifying && 
        (/\d+\s*(melt|burger|fries|fire|chicken)/i.test(lowerMsg) ||
...
exports.handler = createNetlifyHandler(chatHandler);